...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \pause
        \item The frame pointer enables dynamic allocation on the stack  
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        \item Typically the .got.plt entry is filled in \emph{dynamically} by the loader
        \begin{itemize}
            \item that is: when the function is first called
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        \item Typically arguments are in the following order:
        \begin{enumerate}
            \item rdi
            \item rsi
            \item rdx
            \item rcx (system calls: use r10 instead)
            \item r8
            \item r9
            \item stack
        \end{enumerate}
...
\begin{frame}[fragile]
    \frametitle{\texttt{ROPgadget}}
    \texttt{ROPgadget} is another tool to find ROP gadgets

    Usage: \mintinline{bash}{ROPgadget --binary <binary>}
    \docl{https://github.com/JonathanSalwan/ROPgadget}
...
Gadgets information
============================================================
0x0000000000016b68 : adc al, 0 ; add byte ptr [rax], al ; add edx, r8d ; jmp 0x153f0
0x00000000000147a7 : adc al, 0x24 ; mov r12, qword ptr [rsp] ; jmp 0x14719
0x0000000000011627 : adc al, 0x48 ; mov edi, eax ; jmp 0x112a5
...
0x0000000000010153 : xor r9d, r9d ; xor r13d, r13d ; jmp 0xfe7f
0x00000000000168c7 : xor rax, 1 ; jmp 0x153ea
0x000000000000678c : xor rsi, r13 ; jmp 0x5d79

Unique gadgets found: 5771
    \end{codebox}
\end{frame}

\begin{frame}[fragile]
    \frametitle{\texttt{readelf}}
    \texttt{readelf} can be used to find PLT and GOT entries

    Usage: \mintinline{bash}{readelf -rs --wide <binary>}

    Example:
    \begin{codebox}{objdump-nasm}
$ readelf -rs --wide /usr/bin/ls
...
...
117: 0000000000000000     0 FUNC    WEAK   DEFAULT  UND __cxa_finalize@GLIBC_2.2.5 (3)
